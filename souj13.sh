...
do
	...
	1)cat $db;;
        ...
        3)echo "Enter new emp_id "
	        read tid								        echo "Enter Emp_name:"									                read tnm
         echo "Enter designation 
	 read des
	 echo "Enter  emp_salary															                read sal
									          echo "$tid    $tnm    $des    $sal">>$db;;									  4)echo "Enter Id"
	read id		
       	grep -v "$id" $db >dbs1  							 echo "Record is deleted"						      cat dbs1;																											         5)exit;;								         *)echo "Invalid choice ";;
																													     esac
